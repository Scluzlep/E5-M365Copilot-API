...
def setup_paths(session_dir: str):
    global SESSION_DIR, TOKEN_FILE, PROFILE_DIR, LOGIN_LOG, WS_LOG, REPORT
...
def browser_capture(r: Report) -> bool:
    ...
    try:
        with sync_playwright() as pw:
            ...
            def on_ws(ws) -> None:
                is_chat = CHAT_HINT in ws.url or "/Chathub" in ws.url or "/chathub" in ws.url
                if is_chat:
                ...
                def on_recv(*a) -> None:
                    ...
                    if '"event":"appendText"' in text or 'writeAtCursor' in text or '"messages"' in text:
                        summary["append"] = True
                    if '"event":"done"' in text or '{"type":2' in text:
                        summary["done"] = True
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="session",
        help="Directory name for session data.",
    )
    parser.add_argument(
        "--vnc",
        action="store_true",
        help="Launch VNC mode using DISPLAY=:99",
    )
    args = parser.parse_args()

    vnc_mode = args.vnc
    import os
    if sys.platform.startswith("linux") and not vnc_mode and not args.report_only:
    ...
    # Support backwards compatible path handling if "sessions/" is passed
    if "/" not in args.session and args.session != "session":
        args.session = f"sessions/{args.session}"
        
    setup_paths(args.session)
